...
function M.priority(name) return config.format_priority[name] or 0 end

function M.select_client(on_choice)
  local clients = vim.tbl_values(vim.lsp.get_active_clients())
  clients = vim.tbl_filter(function(client) return client.supports_method(FORMATTING_METHOD) end, clients)
  ...
  clients = vim.tbl_filter(function(client) return M.priority(client.name) == max_priority end, clients)
...
function M.on_attach(client, bufnr)
  if not client.supports_method(FORMATTING_METHOD) then return end
  ...
  vim.api.nvim_create_autocmd({ 'BufWritePre' }, {
    ...
    buffer = bufnr,
    callback = function()
      if not config.format_on_save then return end
